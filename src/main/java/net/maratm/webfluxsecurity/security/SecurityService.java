        ...
        return userRepository.findByUsername(username)
                .flatMap(user -> {
                    ...
                    if (!passwordEncoder.matches(password, user.getPassword())) {
                        return Mono.error(new AuthException("Invalid password","MARATM_INVALID_PASSWORD"));
                    }
                    return Mono.just(generateToken(user).toBuilder()
                            .userId(user.getId())
                            .build());
                })
                .switchIfEmpty(Mono.error(new AuthException("Invalid username","MARATM_INVALID_USERNAME")));
    }
